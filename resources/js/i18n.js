import { createI18n } from 'vue-i18n';

const messages = {
    en: {
        // Define your English translations here
        name: 'Name',
        company: 'Client name',
        email: 'Email',
        phone: 'Client number',
        client: 'Client',
        allClients: 'All clients',
        allArticles: 'All articles',
        allReceipts:'All receipts ',
        allRefinements: 'All refinements',
        articleName:'Article Name',
        clientDetails: 'Client details',
        addNewClient :'Add new client',
        addClient: 'Add client',
        clientCreateContactRequired: 'Add at least one contact with name, email, and phone.',
        clientCreateError: 'Could not add client.',
        updateClientModalTitle: 'Edit client',
        updateClientModalHint: 'Company name and address shown on orders and documents.',
        updateClientSave: 'Save changes',
        changeOrderClientTitle: 'Change order customer',
        changeOrderClientHint: 'The order and its line items will be linked to the customer you select. The order contact is set to that customer’s first contact.',
        changeOrderClientSave: 'Update customer',
        changeOrderClientSelectLabel: 'Customer',
        changeOrderClientEmptyPick: 'Select customer',
        changeOrderClientLoading: 'Loading customers…',
        changeOrderClientLoadError: 'Could not load the customer list.',
        changeOrderClientSuccess: 'Customer updated for this order.',
        changeOrderClientError: 'Could not update the customer.',
        cardStatementModalTitle: 'Update client card statement',
        cardStatementModalHint: 'Contact and billing details used for the client card and documents.',
        cardStatementSave: 'Update card statement',
        cardStatementPersonal: 'Personal information',
        cardStatementFinancial: 'Financial information',
        cardStatementAdditional: 'Additional information',
        cardStatementFunction: 'Function / role',
        cardStatementMobile: 'Mobile',
        cardStatementInitialStatement: 'Initial statement',
        cardStatementInitialBalance: 'Initial balance',
        cardStatementCreditLimit: 'Credit limit',
        cardStatementPaymentDeadline: 'Payment deadline (days)',
        cardStatementEdb: 'EDB',
        cardStatementAccount: 'Account',
        cardStatementBank: 'Bank',
        cardStatementFax: 'Fax',
        clientDeleteModalTitle: 'Delete client',
        clientDeleteModalHint: 'Type admin passcode to confirm deleting this client. Only allowed if there are no orders, invoices, or receipts.',
        clientCannotDeleteLinked: 'This client has orders, invoices, or receipts and cannot be deleted.',
        invoice: 'Order',
        invoice2: 'Invoice',
        article: 'Article',
        statement: 'Statement',
        invoiceTitle: 'Order title',
        createNewInvoice: 'Create new order',
        createInvoice: 'Create order',
        comment: 'Comment',
        shippingDetails: 'Shipping details',
        startDate: 'Start date',
        endDate: 'End date',
        dragAndDrop: 'Drag and drop files here',
        browse: 'Browse',
        orderInfo: 'Order info',
        orderLines: 'Order lines',
        image: 'Image',
        width: 'Width',
        height: 'Height',
        length:'Length',
        weight:'Weight',
        color:'Color',
        Kg:'kg',
        M:'m',
        Pcs:'pcs',
        square_meters: 'm²',
        Unit:'Unit',
        Barcode: 'Barcode',
        Delete:'Delete',
        Edit:'Edit',
        receipt:'Receipt',
        refinements: 'Refinements',
        addNewReceipt:'Add new Receipt',
        editReceipt:'Edit Receipt',
        receiptDetails:'Receipt Details',
        warehouse:'Warehouse',
        wItems: 'Warehouse articles',
        toast: {
            success: 'Successfully created order.',
            error: 'Failed to create order.'
        },
        material: 'Material',
        materialList: 'List of all Materials',
        smallMaterial: 'Small Material',
        format: 'Format',
        materials: {
            'Material 1': 'Фолија со црна позадина',
            'Material 2': 'Фолија со бела позадина',
            'Material 3': 'Беклајт фолија',
            'Material 4': 'Форекс 3мм пола табла',
            'Material 5': 'Форекс 3мм',
            'Material 6': 'Форекс 5мм',
            'Material 7': 'Беклајт церада 280цм',
            'Material 8': 'Церада 1м 440гр',
            'Material 9': 'Сити лајт хартија',
            'Material 10': 'Аки плац 5мм пола табла',
            'Material 11': 'Плексиглас',
            'Material 12': 'Аки плац',
            'Material 13': 'Микровал картон',
            'Material 14': 'Каст фолија',
            'Material 15': 'Катер фолија во боја',
            'Material 16': 'Меш платно',
            'Material 17': 'Перфорирана',
            'Material 18': 'Фронт лит церада',
            'Material 19': 'Фронт лит церада 3.2мм',
            'Material 20': 'Бек лит церада',
            'Material 21': 'Блу бек хартија',
            'Material 22': 'Транспарентна фолија мат',
            'Material 23': 'Транспарентна фолија сјај',
            'Material 24': 'Вералит 1мм',
            'Material 25': 'Фибер платно',
            'Material 26': 'Пескарена',
            'Material 27': 'Филм фолија',
            'Material 28': 'Платно за знаме',
        },
        MATERIALS:"MATERIALS",
        materialLargeFormat:'Materials',
        materialSmallFormat: "Material - small format",
        syncJobs: 'Sync selected jobs',
        syncAllJobs: 'Sync all jobs',
        'Job Selection': 'Job Selection',
        'Select jobs to sync with catalog data': 'Select jobs to sync with catalog data',
        'Select All': 'Select All',
        'Clear All': 'Clear All',
        'jobs selected': 'jobs selected',
        'Sync Selected Jobs': 'Sync Selected Jobs', 
        'Please select at least one job to sync': 'Please select at least one job to sync',
        'Please fill in all required fields': 'Please fill in all required fields',
        'Ready to sync': 'Ready to sync',
        'job': 'job',
        'jobs': 'jobs',
        'Syncing...': 'Syncing...',
        'Please select a client before syncing jobs': 'Please select a client before syncing jobs',
        'Configure catalog settings and select jobs to sync': 'Configure catalog settings and select jobs to sync',
        'Select Machine': 'Select Machine',
        'Select Large Material': 'Select Large Material',
        'Select Small Material': 'Select Small Material',
        'Select Action': 'Select Action',
        'Configure catalog settings step by step': 'Configure catalog settings step by step',
        'Machine Selection': 'Machine Selection',
        'Material Selection': 'Material Selection',
        'Large Format Material': 'Large Format Material',
        'Small Format Material': 'Small Format Material',
        'OR': 'OR',
        'Actions & Refinements': 'Actions & Refinements',
        'Enter catalog name': 'Enter catalog name',
        'Enter price': 'Enter price',
        'Enter description (optional)': 'Enter description (optional)',
        'Previous': 'Previous',
        'Next': 'Next',
        'Review & Sync': 'Review & Sync',
        'Sync Summary': 'Sync Summary',
        'Not set': 'Not set',
        'Not selected': 'Not selected',
        'Large Format': 'Large Format',
        'Small Format': 'Small Format',
        'No material selected': 'No material selected',
        'No actions selected': 'No actions selected',
        'No products added yet': 'No products added yet',
        'No services added yet': 'No services added yet',
        'Print': 'Print',
        'Cut': 'Cut',
        'Total Cost': 'Total Cost',
        'Edit Configuration': 'Edit Configuration',
        'Sync Jobs': 'Sync Jobs',
        'Sync Only': 'Sync Only',
        'Back to Edit': 'Back to Edit',
        materialsSmall: {
            'Material small 1': 'Меморандум А4 (80гр)',
            'Material small 2': 'Офсет 80гр / А4',
            'Material small 3': 'Кунздрукт 300гр / 230x330',
            'Material small 4': 'Кунздрукт 350гр / 330x480',
            'Material small 5': 'Кунздрукт 200гр / 320x440',
            'Material small 6': 'Кунздрукт 300гр / 320x440',
            'Material small 7': 'Кунздрукт 300гр / 350x500',
            'Material small 8': 'Кунздрукт 350гр / 350x500',
            'Material small 9': 'Кунздрукт 150гр / 320x440',
            'Material small 10': 'Кунздрукт 130гр / 350x500',
            'Material small 11': 'Кунздрукт 300гр / 210x420',
            'Material small 12': 'Кунздрукт 130гр / 320x440',
            'Material small 13': 'Кунздрукт 300гр / 230x500',
            'Material small 14': 'Кунздрукт 200гр / 290x550',
            'Material small 15': 'Кунздрукт 300гр / 250x350',
            'Material small 16': 'Кунздрукт 250гр / 320x440',
            'Material small 17': 'Офсет 90гр / 350x500',
            'Material small 18': 'Офсет 90гр / 320x440',
            'Material small 19': 'Муфлон 230x330',
            'Material small 20': 'Муфлон 330x480',
            'Material small 21': 'Муфлон 350x500',
            'Material small 22': 'Пенкало metz',
            'Material small 23': 'Пенкало шнидер',
            'Material small 24': 'Запалки ultra',
            'Material small 25': 'Запали itec luss',
            'Material small 26': 'Молив',
            'Material small 27': 'Хромо 270гр 320x440',
            'Material small 28': 'Хромо 270гр 350x500',
            'Material small 29': 'Коверт American',
            'Material small 30': 'Коверт C5',
            'Material small 31': 'Моден картон 260гр 230x330',
            'Material small 32': 'Беџ',
            'Material small 33': 'Пластична ID картичка',
            'Material small 34': 'Кунздрукт 300гр / 330x480',
        },
        machineC: "Machine CUT",
        machineCut: {
            'Machine cut 1': 'Summa cutter',
            'Machine cut 2': 'AHC',
        },
        machineP: "Machine PRINT",
        machinePrint: {
            'Machine print 1': 'Mimaki UV',
            'Machine print 2': 'Pitney bowe s di380',
            'Machine print 3': 'Mimaki eco slovent',
            'Machine print 4': 'Horex iGen 150',
            'Machine print 5': 'Horex Versant 2100',
            'Machine print 6': 'Durst 320',
            'Machine print 7': 'Scitex XL 1500',
            'Machine print 8': 'Durst p10',
            'Machine print 9': 'Cannon B&W imagePress 1125',
            'Machine print 10': 'HP Injet X476W',
        },
        ACTIONS: 'ACTIONS',
        action: 'Action',
        actions: {
            'Action 1': 'Сечење на зунд',
            'Action 2': 'Лепење на форекс 3мм',
            'Action 3': 'Лепење на форекс 5мм',
            'Action 4': 'Лепење на форекс 10мм',
            'Action 5': 'Ковертирање',
            'Action 6': 'Лепење на лепенка',
            'Action 7': 'Монтажа на наша конструкција',
            'Action 8': 'Катирање',
            'Action 9': 'Нитни на банер',
            'Action 10': 'Подвив на банер и врвка',
            'Action 11': 'Ламинација сјај',
            'Action 12': 'Ламинација мат',
            'Action 13': 'Пластификат папка сјај',
            'Action 14': 'Пластификат папка мат',
            'Action 15': 'Ламинат ролна сјај',
            'Action 16': 'Ламинат ролна мат',
            'Action 17': 'Укоричување',
            'Action 18': 'Штанцање (рачно)',
            'Action 19': 'Селективен лак mimaki',
            'Action 20': 'Сечење рачно',
            'Action 21': 'Сечење машинско',
            'Action 22': 'Трансфер',
            'Action 23': 'Да се остави 5-7цм бело',
            'Action 24': 'Да се остави 15цм бело',
            'Action 25': 'Лепење дуплофан (коцка)',
            'Action 26': 'Лепење дуплофан (цела должина)',
            'Action 27': 'Лепење пластика (wobler)',
            'Action 28': 'Хефтање (книга)',
            'Action 29': 'Бигување',
            'Action 30': 'Ламинат ролна сатин',
            'Action 31': 'Бушење',
            'Action 32': 'Лепење на биндер',
            'Action 33': 'Перфорација',
            'Action 34': 'Врзување врвка',
            'Action 35': 'Штанцање (машинско)',
        },
        Quantity:'Quantity',
        Qty:'Qty.',
        Copies:'Copies',
        Code:'Code',
        VAT: 'VAT',
        Tax:'Tax',
        Amount: 'Amount',
        Total: 'Total',
        GeneralInfo:'General Information',
        Product: 'Product',
        Service: 'Service',
        pricelist:'Price List',
        price: 'Price',
        pprice: 'Purchase Price',
        fprice: 'Factory Price',
        additionalInfo: 'Additional Information',
        Shipping:'Shipping',
        materialDetails: 'Material Details',
        articleDetails:'Article Details',
        smallFormatDetails: 'Small Format Details',
        smallFormatMaterialDetails: 'Small Format Material Details',
        addNewMaterialSmall: 'Add Small Format Material',
        quantity: 'Quantity',
        addMaterial: 'Add Material',
        addArticle: 'Add Article',
        addFormat: 'Add Format',
        addSmallMaterial: 'Add Small Material',
        add:'Add',
        createReceipt: 'Create Receipt',
        confirmCreateReceiptTitle: 'Create receipt?',
        confirmCreateReceiptQuestion: 'Are you sure you want to create this receipt?',
        cancel: 'Cancel',
        NewRow:'New Row',
        SmallFormatMaterials: 'Small Format Materials',
        listOfSmallFormat: 'List of Small Format Materials',
        pricePerUnit: 'Price per unit',
        addNewMaterialLarge:'Add Large Format Material',
        addNewArticle:'Add New Article',
        listOfLargeFormat: 'List of Large Format Materials',
        contact: 'Contact',
        contacts: 'Contacts',
        otherContacts: 'Other contacts',
        listOfSmallMaterials: 'List of Small Materials',
        listOfLargeMaterials:'List of Large Materials',
        listOfAllOrders: 'List Of All Orders',
        listOfAllInvoices: 'List Of All Invoiced Orders',
        listOfNotInvoiced:'List of Uninvoiced Orders',
        listOfIncomingInvoices:'List of Incoming Invoices',
        ViewAllInvoices: 'View All Orders',
        UninvoicedOrders: 'Uninvoiced orders',
        listOfAllStatements: 'List Of All Statements',
        invoicedOrders: 'Invoiced orders',
        listofallCardStatements:'List of All Card Statements',
        allSubcategories: 'All subcategories',
        applyFilters: 'Apply filters',
        allCategories: 'All categories',
        CardStatements:'Card Statement',
        CCS:'Client Card Statement',
        bankStatement:'Bank Statement',
        currentReport:'Current Report',
        invoiceGeneration: 'Invoice Generation',
        invoiceEdit:'Edit Invoice',
        InvoiceDetails: 'Order Details',
        InvoiceReview: 'Order Review',
        totalm: 'Total m',
        shippingTo: 'Shipping To',
        jobPriceCost: 'Job Price Cost',
        jobPrice: 'Job Price',
        salePrice:'Sale Price',
        production: 'Production',
        dashboard: 'Dashboard',
        actionInfo: 'Action Overview',
        address: 'Address',
        city: "City",
        Nr:'Nr.',
        from: 'From',
        to: 'To',
        owes: 'Owes',
        requests: 'Requests',
        balance: 'Balance',
        totalBalance: 'Total balance',
        date: 'Date',
        number: 'Number',
        document: 'Document',
        incomingInvoice: 'Incoming invoice',
        outcomeInvoice: 'Outcome invoice',
        statementIncome: 'Statement - income',
        statementExpense: 'Statement - expense',
        analyticsInvoice: 'Analytics for Order - User',
        receiptId: 'Receipt ID',
        other: 'Other',
        actionQuantity: 'Action quantity',
        offerClient: 'Offers for clients',
        offer: 'Offer',
        productionTime: 'Production time',
        analytics: 'Analytics',
        articleAnalytics: 'Analytics for Articles',
        selectChartType: 'Select chart type',
        pie: 'Pie chart',
        bar: 'Bar chart',
        line: 'Line chart',
        error: 'Error',
        detailedBreakdown: 'Detailed breakdown',
        orderCount: 'Order count',
        percentage: 'Percentage',
        clientInvoiceAnalytics: 'Client Invoice Analytics',
        clientInvoiceCostsAnalytics: 'Client Invoice Costs Analytics',
        invoiceCostsDen: 'Invoice Costs (ден.)',
        userInvoiceAnalytics: 'User Invoice Analytics',
        userName: 'User name',
        workerAnalytics: 'Worker Analytics',
        workedId: 'Worker ID',
        totalTimeSpent: 'Total time spent',
        invoicesWorkedOn: 'Invoices worked on',
        selectDateRange: 'Select Date Range',
        year: 'Year',
        month: 'Month',
        day: 'Day',
        reset: 'Reset',
        apply: 'Apply',
        updateArticle: 'Update article',
        close: 'Close',
        articlesPerPage: 'Articles per page',
        fivePerPage: '5 per page',
        tenPerPage: '10 per page',
        twentyPerPage: '20 per page',
        fortyPerPage: '40 per page',
        sixtyPerPage: '60 per page',
        catalogItems: 'Catalog items',
        preview: 'Preview',
        template: 'Template',
        defaultPrice: 'Default price',
        clientPrices: 'Client prices',
        quantityPrices: 'Quantity prices',
        options: 'Options',
        noTemplate: 'No template',
        manage: 'Manage',
        previous: 'Previous',
        page: 'Page',
        of: 'of',
        next: 'Next',
        importFromExcel: 'Import from Excel',
        exportToExcel: 'Export to Excel',
        status: 'Status',
        changeFile: 'Change file',
        batchArticleImport: 'Batch article import',
        mapColumns: 'Map Excel columns to article fields',
        previewImport: 'Preview import',
        toCreate: 'To create',
        duplicate: 'Duplicate',
        backToMapping: 'Back to mapping',
        execute: 'Execute',
        noRowsToPreview: 'No data rows to preview. Check that the name column has values and the file has data rows.',
        importStatus: 'Import status',
        willCreate: 'Will create article',
        willUpdate: 'Will update stock',
        willDrop: 'Will be dropped',
        willSkip: 'will be skipped',
        selectAllRows: 'Select all rows',
        noActionsAvailableForThisItem: 'No actions available for this item',
        description: 'Description',
        selectMachine: 'Select machine',
        category: 'Category',
        selectCategory: 'Select category',
        clearSelection: 'Clear selection',
        fileUpload: 'File upload',
        orClickToBrowse: 'or click to browse',
        supportedFormats: 'Supported formats',
        clickOrDragToUpdateImage: 'Click or drag to update image',
        templateFilePdfOnly: 'Template File (PDF only)',
        dragAndDropTemplatePdfHere: 'Drag and drop template PDF here',
        removeTemplate: 'Remove template',
        forOffer: 'For offer',
        forSales: 'For sales',
        componentArticles: 'Component articles',
        product: 'Product',
        products: 'Products',
        addProduct: 'Add product',
        service: 'Service',
        services: 'Services',
        addService: 'Add service',
        costSummary: 'Cost summary',
        productsCost: 'Products cost',
        servicesCost: 'Services cost',
        totalCostPrice: 'Total cost price',
        addAction: 'Add action',
        updateCatalogItem: 'Update catalog item',
        addClientPrice: 'Add client price',
        currentClientPrices: 'Current client prices',
        manageQuantityPrices: 'Manage quantity prices',
        manageClientPrices: 'Manage client prices',
        quantityFrom: 'Quantity from',
        quantityTo: 'Quantity to',
        addQuantityPrice: 'Add quantity price',
        currentQuantityPrices: 'Current quantity prices',
        quantityRange: 'Quantity range',
        templatePreview: 'Template preview',
        catalogItemCreation: 'Catalog item creation',
        createNewSubcategory: 'Create new subcategory',
        clearFilters: 'Clear filters',
        subcategory: 'Subcategory',
        selectSubcategory: 'Select subcategory',
        viewSubcategories: 'View subcategories',
        manageSubcategories: 'Manage subcategories',
        createCatalogItem: 'Create catalog item',
        createNewCatalogItem: 'Create new catalog item',
        clientsPerPage: 'Clients per page',
        showAll: 'Show all',
        newContact: 'New contact',
        update: 'Update',
        cardStatement: 'Card statement',
        viewContacts: 'View contacts',
        delete: 'Delete',
        contactsFor: 'Contacts for',
        addNewContact: 'Add new contact',
        addContactModalHint: 'Enter details for one contact.',
        addContactValidation: 'Please fill in name, email, and phone.',
        addContactSuccess: 'Contact saved successfully.',
        addContactError: 'Could not save contact.',
        viewContactsModalHint: 'People linked to this client.',
        noContactsYet: 'No contacts yet — add one below.',
        contactDeletedSuccess: 'Contact removed.',
        contactDeletedError: 'Could not remove contact.',
        saveContact: 'Save contact',
        addNewMaterial: 'Add new material',
        exitEditMode: 'Exit edit mode',
        editMode: 'Edit mode',
        save: 'Save',
        search: 'Search',
        perPage: 'Per page',
        printMaterials: 'Print materials',
        printAllMaterials: 'Print all materials',
        editClientPrice: 'Edit client price',
        updateClientPrice: 'Update client price',
        updateClientSpecificPrice: 'Update client-specific price',
        createClientSpecificPrice: 'Create client-specific price',
        selectCatalogItem: 'Select catalog item',
        default: 'Default',
        selectClient: 'Select client',
        clientSpecificPrice: 'Client-specific price',
        overrideDefaultPriceMessage: 'This price will override the default price for this client',
        clientPrice: 'Client price',
        catalogItem: 'Catalog item',
        create: 'Create',
        createClientPrice: 'Create client price',
        addNewClientSpecificPrice: 'Add new client-specific price',
        customPrice: 'Custom price',
        confirmDelete: 'Confirm delete',
        areYouSureDeletePriceMessage: 'Are you sure you want to delete this price?',
        cancel: 'Cancel',
        noClientPricesFound: 'No client-specific prices found.',
        addOneNow: 'Add one now',
        forClient: 'for client',
        thisActionCannotBeUndoneMessage: 'This action cannot be undone.',
        deleting: 'Deleting...',
        manageCustomPrices: 'Manage custom prices for specific clients',
        clientSpecificPrices: 'Client-Specific Prices',
        filterWarehouse: 'Filter warehouse',
        allWarehouses: 'All warehouse',
        viewWarehouses: 'View warehouses',
        listOfAllTheWarehouses: 'List of all the warehouses',
        addWarehouse: 'Add warehouse',
        addNewWarehouse: 'Add new warehouse',
        order: 'Order',
        customer: 'Customer',
        createdBy: 'Created by',
        currentStep: 'Current step',
        img: 'Img',
        qty: 'Qty',
        copies: 'Copies',
        ln: 'LN',
        print: 'Print',
        cut: 'Cut',
        nextStep: 'Next step',
        startJob: 'Start job',
        endJob: 'End job',
        readNotes: 'Read notes before you can process this.',
        completed: 'COMPLETED',
        waitingStatus: 'Waiting status',
        thisOrderIsOnHold: 'This order is on hold',
        readNote: 'Read note',
        acknowledge: 'Acknowledge',
        totalActiveJobs: 'Total active jobs',
        totalPendingJobs: 'Total pending jobs',
        totalJobsOnHold: 'Total jobs ON HOLD',
        highPriorityJobs: 'High priority jobs',
        online: 'Online',
        offline: 'Offline',
        viewMachines: 'View machines',
        listOfAllTheMachines: 'List of all the machines',
        addMachine: 'Add machine',
        addNewMachine: 'Add new machine',
        type: 'Type',
        searchCatalogItem: 'Search catalog item',
        searchClient: 'Search client',
        pricingMethod: 'Pricing Method',
        priceByQuantity: 'Price by Quantity',
        priceByCopies: 'Price by Copies',
        materialImportSummary: 'Material Import Summary',
        TotalImported: 'Total Imported',
        CurrentStock: 'Current Stock',
        TotalValue: 'Total Value',
        AvgPrice: 'Average Price',
        LastImport: 'Last Import',
        Format: 'Format',
        Dimensions: 'Dimensions',
        SummaryStatistics: 'Summary Statistics',
        TotalMaterials: 'Total Materials',
        TotalImportedQuantity: 'Total Imported Quantity',
        CurrentStockValue: 'Current Stock Value',
        tradeInvoice: 'Trade Invoice',
        createTradeInvoice: 'Create Trade Invoice',
        tradeInvoiceDetails: 'Trade Invoice Details',
        NewRow: 'New Row',
        Delete: 'Delete',
        LoadArticles: 'Load Articles',
        Available: 'Available',
        notes: 'Notes',
        addNotesHere: 'Add notes here',
        allTradeInvoices: 'All Trade Invoices',
        financeTabs: {
            allInvoices: 'All Invoices',
            notInvoiced: 'Not Invoiced',
            incomingInvoice: 'Incoming Invoice',
            tradeInvoices: 'Trade Invoices',
            receipts: 'Receipts',
            individual: 'Individual',
            stockRealization: 'Stock Realization',
            bankStatements: 'Bank Statements',
            clientStatements: 'Client Statements',
        },
        financeTable: {
            index: '#',
            invoice: 'Invoice',
            date: 'Date',
            client: 'Client',
            amount: 'Amount',
            tax: 'Tax',
            total: 'Total',
            createdBy: 'Created By',
            orders: 'Orders',
            comment: 'Comment',
            actions: 'Actions',
            warehouse: 'Warehouse',
            cost: 'Cost',
            bill: 'Bill',
            costType: 'Cost type',
            billType: 'Bill type',
        },
    },
    mk: {
        // Define your Macedonian translations here
        greeting: 'Здраво',
        name: 'Име',
        company: 'Име на клиент',
        email: 'Е-пошта',
        phone: 'Телефон',
        client: 'Клиент',
        allClients: 'Сите клиенти',
        allArticles: 'Сите артикли',
        allReceipts: 'Сите приемници',
        allRefinements:'Сите доработки',
        editReceipt:'Уреди приемница',
        articleName: 'Име на артикл',
        clientDetails: 'Детали за клиент',
        addNewClient :'Додади нов клиент',
        addClient: 'Додади клиент',
        clientCreateContactRequired: 'Додадете барем еден контакт со име, е-пошта и телефон.',
        clientCreateError: 'Не успеа додавањето на клиент.',
        updateClientModalTitle: 'Уреди клиент',
        updateClientModalHint: 'Име и адреса се користат на налози и документи.',
        updateClientSave: 'Зачувај промени',
        changeOrderClientTitle: 'Смени клиент на налог',
        changeOrderClientHint: 'Налогот и ставките ќе се поврзат со избраниот клиент. Контактот на налогот се поставува на првиот контакт на тој клиент.',
        changeOrderClientSave: 'Зачувај клиент',
        changeOrderClientSelectLabel: 'Клиент',
        changeOrderClientEmptyPick: 'Избери клиент',
        changeOrderClientLoading: 'Се вчитуваат клиентите…',
        changeOrderClientLoadError: 'Не успеа вчитувањето на листата на клиенти.',
        changeOrderClientSuccess: 'Клиентот на налогот е ажуриран.',
        changeOrderClientError: 'Не успеа промената на клиентот.',
        cardStatementModalTitle: 'Клиентска картичка',
        cardStatementModalHint: 'Контакт и финансиски податоци за картичката и документи.',
        cardStatementSave: 'Зачувај картичка',
        cardStatementPersonal: 'Лични податоци',
        cardStatementFinancial: 'Финансиски податоци',
        cardStatementAdditional: 'Дополнително',
        cardStatementFunction: 'Функција',
        cardStatementMobile: 'Мобилен',
        cardStatementInitialStatement: 'Почетен извод',
        cardStatementInitialBalance: 'Почетно салдо',
        cardStatementCreditLimit: 'Кредитен лимит',
        cardStatementPaymentDeadline: 'Рок на плаќање (денови)',
        cardStatementEdb: 'ЕДБ',
        cardStatementAccount: 'Сметка',
        cardStatementBank: 'Банка',
        cardStatementFax: 'Факс',
        clientDeleteModalTitle: 'Избриши клиент',
        clientDeleteModalHint: 'Внесете админ лозинка за бришење на клиентот. Дозволено само ако нема налози, фактури или приемници.',
        clientCannotDeleteLinked: 'Овој клиент има налози, фактури или приемници и не може да се избрише.',
        invoice: 'Налог',
        invoice2:'Фактура',
        statement: 'Извод',
        article: 'Артикл',
        invoiceTitle: 'Име на налог',
        createNewInvoice: 'Креирај нов налог',
        createNewSubcategory: 'Креирај нова подкатегорија',
        selectSubcategory: 'Избери подкатегорија',
        viewSubcategories: 'Погледни подкатегории',
        manageSubcategories: 'Подкатегории',
        subcategory: 'Подкатегорија',
        allSubcategories: 'Сите подкатегории',
        allCategories: 'Сите категории',
        clearFilters: 'Освежи',
        applyFilters: 'Прикажи',
        createInvoice: 'Креирај налог',
        comment: 'Коментар',
        shippingDetails: 'Детали за испорака',
        startDate: 'Почетен датум',
        endDate: 'Краен датум',
        dragAndDrop: 'Прикачи документи тука',
        browse: 'Пребарај',
        orderInfo: 'Информации за налог',
        orderLines: 'Налог и ставки',
        image: 'Слика',
        width: 'Ширина',
        height: 'Висина',
        length:'Должина',
        weight:'Тежина',
        color:'Боја',
        Kg:'кг',
        M:'м',
        Pcs:'ком',
        square_meters: 'м²',
        Unit:'Единица Мерка',
        Barcode: 'Бар Код',
        Delete:'Избриши',
        Edit:'Измени',
        receipt:'Приемница',
        refinements:'Доработки',
        addNewReceipt:'Додади нова приемница',
        receiptDetails:'Детали за приемница',
        warehouse: 'Магацин',
        wItems:'Магацински артикли',
        toast: {
            success: 'Налогот е успешно креиран.',
            error: 'Настана грешка при креирање на налогот.'
        },
        material: 'Материјал',
        materialList: 'Листа на сите материјали',
        smallMaterial: 'Мал Материјал',
        format: 'Формат',
        materials: {
            'Material 1': 'Фолија со црна позадина',
            'Material 2': 'Фолија со бела позадина',
            'Material 3': 'Беклајт фолија',
            'Material 4': 'Форекс 3мм пола табла',
            'Material 5': 'Форекс 3мм',
            'Material 6': 'Форекс 5мм',
            'Material 7': 'Беклајт церада 280цм',
            'Material 8': 'Церада 1м 440гр',
            'Material 9': 'Сити лајт хартија',
            'Material 10': 'Аки плац 5мм пола табла',
            'Material 11': 'Плексиглас',
            'Material 12': 'Аки плац',
            'Material 13': 'Микровал картон',
            'Material 14': 'Каст фолија',
            'Material 15': 'Катер фолија во боја',
            'Material 16': 'Меш платно',
            'Material 17': 'Перфорирана',
            'Material 18': 'Фронт лит церада',
            'Material 19': 'Фронт лит церада 3.2мм',
            'Material 20': 'Бек лит церада',
            'Material 21': 'Блу бек хартија',
            'Material 22': 'Транспарентна фолија мат',
            'Material 23': 'Транспарентна фолија сјај',
            'Material 24': 'Вералит 1мм',
            'Material 25': 'Фибер платно',
            'Material 26': 'Пескарена',
            'Material 27': 'Филм фолија',
            'Material 28': 'Платно за знаме',
        },
        MATERIALS:"МАТЕРИЈАЛИ",
        materialLargeFormat:'Материјали',
        materialSmallFormat: "Материјал - мал формат",
        syncJobs: 'Ажурирај',
        syncAllJobs: 'Ажурирај ги сите',
        'Job Selection': 'Работа за синхронизација',
        'Select jobs to sync with catalog data': 'Избери задачи за синхронизација со податоци од каталог',
        'Select All': 'Избери сите',
        'Clear All': 'Освежи',
        'jobs selected': 'задачи избрани',
        'Sync Selected Jobs': 'Синхронизирај избрани задачи', 
        'Please select at least one job to sync': 'Ве молам изберете барем една задача за синхронизација',
        'Please fill in all required fields': 'Ве молам пополнете сите задолжени полиња',
        'Ready to sync': 'Готово за синхронизација',
        'job': 'задача',
        'jobs': 'задачи',
        'Syncing...': 'Синхронизирам...',
        'Please select a client before syncing jobs': 'Ве молам изберете клиент пред да синхронизирате задачи',
        'Configure catalog settings and select jobs to sync': 'Конфигурирај параметри на каталогот и избери задачи за синхронизација',
        'Select Machine': 'Избери машина',
        'Select Large Material': 'Избери голем материјал',
        'Select Small Material': 'Избери мал материјал',
        'Select Action': 'Избери акција',
        'Configure catalog settings step by step': 'Конфигурирај параметри чекор по чекор',
        'Machine Selection': 'Избор на машини',
        'Material Selection': 'Избор на материјал',
        'Large Format Material': 'Материјал - голем формат',
        'Small Format Material': 'Материјал - мал формат',
        'OR': 'ИЛИ',
        'Actions & Refinements': 'Акции и доработки',
        'Enter catalog name': 'Внеси име на каталог',
        'Enter price': 'Внеси цена',
        'Enter description (optional)': 'Внеси опис (опционално)',
        'Previous': 'Претходно',
        'Next': 'Следно',
        'Review & Sync': 'Преглед и синхронизација',
        'Sync Summary': 'Преглед на синхронизација',
        'Not set': 'Не е поставено',
        'Not selected': 'Не е избрано',
        'Large Format': 'Голем формат',
        'Small Format': 'Мал формат',
        'No material selected': 'Нема избран материјал',
        'No actions selected': 'Нема избрани акции',
        'No products added yet': 'Сè уште нема додадени производи',
        'No services added yet': 'Сè уште нема додадени услуги',
        'Print': 'Печатење',
        'Cut': 'Сечење',
        'Total Cost': 'Вкупен трошок',
        'Edit Configuration': 'Уреди конфигурација',
        'Sync Jobs': 'Синхронизирај задачи',
        'Sync Only': 'Синхронизирај само',
        'Back to Edit': 'Врати се на уредување',
        materialsSmall: {
            'Material small 1': 'Меморандум А4 (80гр)',
            'Material small 2': 'Офсет 80гр / А4',
            'Material small 3': 'Кунздрукт 300гр / 230x330',
            'Material small 4': 'Кунздрукт 350гр / 330x480',
            'Material small 5': 'Кунздрукт 200гр / 320x440',
            'Material small 6': 'Кунздрукт 300гр / 320x440',
            'Material small 7': 'Кунздрукт 300гр / 350x500',
            'Material small 8': 'Кунздрукт 350гр / 350x500',
            'Material small 9': 'Кунздрукт 150гр / 320x440',
            'Material small 10': 'Кунздрукт 130гр / 350x500',
            'Material small 11': 'Кунздрукт 300гр / 210x420',
            'Material small 12': 'Кунздрукт 130гр / 320x440',
            'Material small 13': 'Кунздрукт 300гр / 230x500',
            'Material small 14': 'Кунздрукт 200гр / 290x550',
            'Material small 15': 'Кунздрукт 300гр / 250x350',
            'Material small 16': 'Кунздрукт 250гр / 320x440',
            'Material small 17': 'Офсет 90гр / 350x500',
            'Material small 18': 'Офсет 90гр / 320x440',
            'Material small 19': 'Муфлон 230x330',
            'Material small 20': 'Муфлон 330x480',
            'Material small 21': 'Муфлон 350x500',
            'Material small 22': 'Пенкало metz',
            'Material small 23': 'Пенкало шнидер',
            'Material small 24': 'Запалки ultra',
            'Material small 25': 'Запали itec luss',
            'Material small 26': 'Молив',
            'Material small 27': 'Хромо 270гр 320x440',
            'Material small 28': 'Хромо 270гр 350x500',
            'Material small 29': 'Коверт American',
            'Material small 30': 'Коверт C5',
            'Material small 31': 'Моден картон 260гр 230x330',
            'Material small 32': 'Беџ',
            'Material small 33': 'Пластична ID картичка',
            'Material small 34': 'Кунздрукт 300гр / 330x480',
        },
        machineC: "Машини CUT",
        machineCut: {
            'Machine cut 1': 'Summa cutter',
            'Machine cut 2': 'AHC',
        },
        machineP: "Машини PRINT",
        machinePrint: {
            'Machine print 1': 'Mimaki UV',
            'Machine print 2': 'Pitney bowe s di380',
            'Machine print 3': 'Mimaki eco slovent',
            'Machine print 4': 'Horex iGen 150',
            'Machine print 5': 'Horex Versant 2100',
            'Machine print 6': 'Durst 320',
            'Machine print 7': 'Scitex XL 1500',
            'Machine print 8': 'Durst p10',
            'Machine print 9': 'Cannon B&W imagePress 1125',
            'Machine print 10': 'HP Injet X476W',

        },
        ACTIONS:'ДОРАБОТКИ',
        action: 'Доработка',
        actions: {
            'Action 1': 'Сечење на зунд',
            'Action 2': 'Лепење на форекс 3мм',
            'Action 3': 'Лепење на форекс 5мм',
            'Action 4': 'Лепење на форекс 10мм',
            'Action 5': 'Ковертирање',
            'Action 6': 'Лепење на лепенка',
            'Action 7': 'Монтажа на наша конструкција',
            'Action 8': 'Катирање',
            'Action 9': 'Нитни на банер',
            'Action 10': 'Подвив на банер и врвка',
            'Action 11': 'Ламинација сјај',
            'Action 12': 'Ламинација мат',
            'Action 13': 'Пластификат папка сјај',
            'Action 14': 'Пластификат папка мат',
            'Action 15': 'Ламинат ролна сјај',
            'Action 16': 'Ламинат ролна мат',
            'Action 17': 'Укоричување',
            'Action 18': 'Штанцање (рачно)',
            'Action 19': 'Селективен лак mimaki',
            'Action 20': 'Сечење рачно',
            'Action 21': 'Сечење машинско',
            'Action 22': 'Трансфер',
            'Action 23': 'Да се остави 5-7цм бело',
            'Action 24': 'Да се остави 15цм бело',
            'Action 25': 'Лепење дуплофан (коцка)',
            'Action 26': 'Лепење дуплофан (цела должина)',
            'Action 27': 'Лепење пластика (wobler)',
            'Action 28': 'Хефтање (книга)',
            'Action 29': 'Бигување',
            'Action 30': 'Ламинат ролна сатин',
            'Action 31': 'Бушење',
            'Action 32': 'Лепење на биндер',
            'Action 33': 'Перфорација',
            'Action 34': 'Врзување врвка',
            'Action 35': 'Штанцање (машинско)',
        },
        Quantity:'Количина',
        Qty:'Кол.',
        Copies:'Копии',
        Code:'Шифра',
        GeneralInfo: 'Основни Податоци',
        Product: 'Производ',
        Service: 'Услуга',
        pricelist: 'Ценовник',
        price: 'Цена',
        pprice: 'Цена Набавна',
        fprice: 'Цена Фабричка',
        additionalInfo:'Дополнителни информации',
        VAT:'ДДВ',
        Tax: 'Данок',
        Amount:'Износ',
        Total: 'Вкупно',
        Shipping:'Достава',
        materialDetails: 'Детали за материјал',
        articleDetails: 'Детали за артикл',
        smallFormatDetails: 'Детали за мал формат',
        smallFormatMaterialDetails: 'Детали за мал формат материајал',
        addNewMaterialSmall: 'Додади материјал - мал формат',
        quantity: 'Количина',
        addMaterial: 'Додади материјал',
        addArticle: 'Додади артикл',
        addFormat: 'Додади формат',
        addSmallMaterial: 'Додади мал материјал',
        add:'Додади',
        createReceipt: 'Креирај приемница',
        confirmCreateReceiptTitle: 'Креирање приемница?',
        confirmCreateReceiptQuestion: 'Дали сте сигурни дека сакате да ја креирате оваа приемница?',
        cancel: 'Откажи',
        NewRow:'Нов Ред',
        SmallFormatMaterials: 'Материјали - мал формат',
        listOfSmallFormat: 'Листа на материјали - мал формат',
        listOfSmallMaterials: 'Листа на Мали Материјали',
        listOfLargeMaterials:'Листа на Големи Материјали',
        pricePerUnit: 'Цена по парче',
        contact: 'Контакт',
        contacts: 'Контакти',
        otherContacts: 'Останати контакти',
        production: 'Продукција',
        dashboard: 'Преглед',
        actionInfo: 'Преглед на акција',
        addNewMaterialLarge:'Додади материјал - голем формат',
        addNewArticle:'Додади нов артикл',
        listOfLargeFormat: 'Листа на материјали - голем формат',
        listOfAllOrders: 'Листа на сите нарачки',
        listOfAllInvoices: 'Листа на сите фактурирани нарачки',
        listOfNotInvoiced:'Листа на нефактурирани нарачки',
        listOfAllStatements:'Листа на сите Изводи',
        ViewAllInvoices: 'Погледни ги сите нарачки',
        UninvoicedOrders:'Нефактурирани нарачки',
        invoicedOrders:'Фактурирани нарачки',
        listofallCardStatements:'Листа на сите извод картици',
        listOfIncomingInvoices:'Листа на влезни фактури',
        CardStatements:'Извод Карица',
        CCS:'Извод картица на клиент',
        bankStatement: 'Извод банка',
        currentReport:'Тековен извештај',
        invoiceGeneration:'Генерирање на фактура',
        invoiceEdit: 'Уреди фактура',
        InvoiceDetails: 'Детали за нарачка',
        InvoiceReview: 'Преглед на нарачка',
        totalm: 'Вкупно м.',
        shippingTo: 'Достава до',
        jobPriceCost: 'Цена на продукт (потрошено)',
        jobPrice: 'Цена на продукт',
        salePrice: 'Продажна цена',
        address: 'Адреса',
        city: "Град",
        Nr:'Бр.',
        from: 'Од',
        to: 'До',
        owes: 'Должи',
        requests: 'Платил',
        balance: 'Баланс',
        totalBalance: 'Вкупен баланс',
        date: 'Датум',
        number: 'Број',
        document: 'Документ',
        incomingInvoice: 'Влезна фактура',
        outcomeInvoice: 'Излезна фактура',
        statementIncome: 'Извод - уплата',
        statementExpense: 'Извод - исплата',
        analyticsInvoice: 'Аналитика на налози - корисници',
        receiptId: 'Приемница бр.',
        other: 'Друго',
        actionQuantity: 'Количина доработка',
        offerClient: 'Понуди за клиенти',
        offer: 'Понуда',
        productionTime: 'Време на изработка',
        analytics: 'Аналитика',
        articleAnalytics: 'Аналитика на артикли',
        selectChartType: 'Избери тип на график',
        pie: 'Пита - график',
        bar: 'Бар - график',
        line: 'Линија - график',
        error: 'Грешка',
        detailedBreakdown: 'Детален преглед',
        orderCount: 'Број на налози',
        percentage: 'Процент',
        clientInvoiceAnalytics: 'Аналитика на налози - клиенти',
        clientInvoiceCostsAnalytics: 'Аналитика на трошоци на налози - клиенти',
        invoiceCostsDen: 'Трошоци за налог (ден.)',
        userInvoiceAnalytics: 'Аналитика на налози - корисници',
        userName: 'Име на корисник',
        workerAnalytics: 'Аналитика на работници',
        workedId: 'Работник бр.',
        totalTimeSpent: 'Вкупно време потрошено',
        invoicesWorkedOn: 'Број на изработени налози',
        selectDateRange: 'Избери датум',
        year: 'Година',
        month: 'Месец',
        day: 'Ден',
        reset: 'Ресетирај',
        apply: 'Филтрирај',
        updateArticle: 'Ажурирај го артиклот',
        close: 'Затвори',
        articlesPerPage: 'Артикли по страна',
        fivePerPage: '5 по страна',
        tenPerPage: '10 по страна',
        twentyPerPage: '20 по страна',
        fortyPerPage: '40 по страна',
        sixtyPerPage: '60 по страна',
        catalogItems: 'Ставки на каталог',
        preview: 'Преглед',
        template: 'Темплејт',
        defaultPrice: 'Основна цена',
        clientPrices: 'Цени по клиент',
        quantityPrices: 'Цени по количина',
        options: 'Опции',
        noTemplate: 'Нема темплејт',
        manage: 'Менаџирај',
        previous: 'Претходна',
        page: 'Страна',
        of: 'од',
        next: 'Следна',
        importFromExcel: 'Увези од Excel',
        exportToExcel: 'Извези во Excel',
        changeFile: 'Измени фајл',
        status: 'Статус',
        batchArticleImport: 'Групен увоз на артикли',
        mapColumns: 'Мапирај Excel колони со полиња',
        previewImport: 'Преглед на увозот',
        toCreate: 'за креирање',
        duplicate: 'дупликати',
        backToMapping: 'Назад на мапирање',
        execute: 'Изврши',
        noRowsToPreview: 'Нема редови за преглед. Проверете дали колоната за име има вредности и дали фајлот има податоци.',
        importStatus: 'Статус на увоз',
        willCreate: 'Ќе креира артикл',
        willUpdate: 'Ќе ажурира залиха',
        willDrop: 'Ќе се прескокне',
        willSkip: 'ќе се прескокне',
        selectAllRows: 'Селектирај ги сите редови',
        noActionsAvailableForThisItem: 'Нема достапни акции за оваа ставка',
        description: 'Опис',
        selectMachine: 'Избери машина',
        category: 'Категорија',
        selectCategory: 'Избери категорија',
        clearSelection: 'Избриши филтри',
        fileUpload: 'Прикачи фајл',
        orClickToBrowse: 'или кликни за да пребараш',
        supportedFormats: 'Поддржани формати',
        clickOrDragToUpdateImage: 'Кликни или повлечи за да одбереш слика',
        templateFilePdfOnly: 'Темплејт фајл (само PDF)',
        dragAndDropTemplatePdfHere: 'Повлечи темплејт PDF тука',
        removeTemplate: 'Отстрани темплејт',
        forOffer: 'За понуда',
        forSales: 'За продажба',
        componentArticles: 'Компонент артикли',
        product: 'Продукт',
        products: 'Продукти',
        addProduct: 'Додади продукт',
        service: 'Сервис',
        services: 'Сервиси',
        addService: 'Додади сервис',
        costSummary: 'Преглед на трошоци',
        productsCost: 'Трошоци на продукти',
        servicesCost: 'Трошоци на сервиси',
        totalCostPrice: 'Вкупна цена на трошок',
        addAction: 'Додади акција',
        updateCatalogItem: 'Ажурирај ставка од каталог',
        addClientPrice: 'Додади цена по клиент',
        currentClientPrices: 'Моментални цени на клиент',
        manageQuantityPrices: 'Менаџирај со цени по количина',
        manageClientPrices: 'Менаџирај со цени по клиент',
        quantityFrom: 'Количина од',
        quantityTo: 'Количина до',
        addQuantityPrice: 'Додади цена по количина',
        currentQuantityPrices: 'Моментални цени по количина',
        quantityRange: 'Ранг на количини',
        templatePreview: 'Преглед на темплејт',
        catalogItemCreation: 'Креирање на ставка од каталог',
        createCatalogItem: 'Креирај ставка од каталог',
        createNewCatalogItem: 'Креирај нова ставка од каталог',
        clientsPerPage: 'Клиенти по страна',
        showAll: 'Покажи ги сите',
        newContact: 'Нов контакт',
        update: 'Ажурирај',
        cardStatement: 'Клиентска картичка',
        viewContacts: 'Види контакти',
        delete: 'Избриши',
        contactsFor: 'Контакти за',
        addNewContact: 'Додади нов контакт',
        addContactModalHint: 'Внесете податоци за еден контакт.',
        addContactValidation: 'Пополнете име, е-пошта и телефон.',
        addContactSuccess: 'Контактот е зачуван.',
        addContactError: 'Не успеа зачувувањето.',
        viewContactsModalHint: 'Луѓе поврзани со овој клиент.',
        noContactsYet: 'Нема контакти — додадете подолу.',
        contactDeletedSuccess: 'Контактот е избришан.',
        contactDeletedError: 'Не успеа бришењето.',
        saveContact: 'Зачувај контакт',
        addNewMaterial: 'Додади нов материјал',
        exitEditMode: 'Излези од модот за промени',
        editMode: 'Промени',
        save: 'Зачувај',
        search: 'Пребарај',
        perPage: 'По страна',
        printMaterials: 'Испечати материјали',
        printAllMaterials: 'Испечати ги сите материјали',
        editClientPrice: 'Промени цена по клиент',
        updateClientPrice: 'Ажурирај цена по клиент',
        updateClientSpecificPrice: 'Ажурирај цена на специфичен клиент',
        createClientSpecificPrice: 'Креирај цена на специфичен клиент',
        selectCatalogItem: 'Избери ставка од каталог',
        default: 'Основно',
        selectClient: 'Избери клиент',
        clientSpecificPrice: 'Цена на специфичен клиент',
        overrideDefaultPriceMessage: 'Оваа цена ќе ја замени основната цена на клиентот',
        clientPrice: 'Цена за клиент',
        catalogItem: 'Ставка од каталог',
        create: 'Креирај',
        createClientPrice: 'Креирај цена за клиент',
        addNewClientSpecificPrice: 'Додади нова цена за специфичен клиент',
        customPrice: 'Различна цена',
        confirmDelete: 'Потврди бришење',
        areYouSureDeletePriceMessage: 'Дали сте сигурни дека сакате да ја избришете цената за',
        cancel: 'Откажи',
        noClientPricesFound: 'No client-specific prices found.',
        addOneNow: 'Додади цена веднаш',
        forClient: 'за клиент',
        thisActionCannotBeUndoneMessage: 'Откако еднаш ќе се изврши, оваа акција не може да се откаже',
        deleting: 'Бришење...',
        manageCustomPrices: 'Менаџирање со цените за клиент',
        clientSpecificPrices: 'Цени за клиент',
        filterWarehouse: 'Филтрирај магацини',
        allWarehouses: 'Сите магацини',
        viewWarehouses: 'Види магацини',
        listOfAllTheWarehouses: 'Листа на сите магацини',
        addWarehouse: 'Додади магацин',
        addNewWarehouse: 'Додади нов магацин',
        order: 'Нарачка',
        customer: 'Купувач',
        createdBy: 'Креирано од',
        currentStep: 'Моментален чекор',
        img: 'Слика',
        qty: 'Кол.',
        copies: 'Копии',
        ln: 'LN',
        print: 'Принтање',
        cut: 'Сечење',
        nextStep: 'Следен чекор',
        startJob: 'Започни задача',
        endJob: 'Заврши задача',
        readNotes: 'Прочитај ги забелешките пред да продолжиш.',
        completed: 'ЗАВРШЕНО',
        waitingStatus: 'Статус на чекање',
        thisOrderIsOnHold: 'Оваа нарачка е паузирана',
        readNote: 'Прочитај забелешки',
        acknowledge: 'Прочитано',
        totalActiveJobs: 'Вкупно активни задачи',
        totalPendingJobs: 'Вкупно незапочнати задачи',
        totalJobsOnHold: 'Вкупно паузирани задачи',
        highPriorityJobs: 'Задачи со висок приоритет',
        online: 'Активна',
        offline: 'Неактивна',
        viewMachines: 'Погледни машини',
        listOfAllTheMachines: 'Листа на сите машини',
        addMachine: 'Додади машина',
        addNewMachine: 'Додади нова машина',
        type: 'Тип',
        searchCatalogItem: 'Пребарај ставка од каталог',
        searchClient: 'Пребарај клиент',
        pricingMethod: 'Начин на пресметување на цена',
        priceByQuantity: 'Цена по количина',
        priceByCopies: 'Цена по копии',
        materialImportSummary: 'Преглед на увоз на материјали',
        TotalImported: 'Вкупно увезено',
        CurrentStock: 'Моментална залиха',
        TotalValue: 'Вкупна вредност',
        AvgPrice: 'Просечна цена',
        LastImport: 'Последен увоз',
        Format: 'Формат',
        Dimensions: 'Димензии',
        SummaryStatistics: 'Статистика',
        TotalMaterials: 'Вкупно материјали',
        TotalImportedQuantity: 'Вкупна увезена количина',
        CurrentStockValue: 'Вредност на моментална залиха',
        tradeInvoice: 'Торговска Фактура',
        createTradeInvoice: 'Креирај торговска фактура',
        tradeInvoiceDetails: 'Детали за торговска фактура',
        NewRow: 'Нов ред',
        Delete: 'Избриши',
        LoadArticles: 'Најди артикли',
        Available: 'Наоѓање на артикли',
        notes: 'Забелешки',
        addNotesHere: 'Додади забелешки тука',
        allTradeInvoices: 'Сите торговски фактури',
        financeTabs: {
            allInvoices: 'Фактурирани',
            notInvoiced: 'Не фактурирани',
            incomingInvoice: 'Влезни фактури',
            tradeInvoices: 'Трговија',
            receipts: 'Приемници',
            individual: 'Физичко',
            stockRealization: 'Реализација',
            bankStatements: 'Изводи',
            clientStatements: 'Клиентски картици',
        },
        financeTable: {
            index: '#',
            invoice: 'Фактура',
            date: 'Датум',
            client: 'Клиент',
            amount: 'Износ',
            tax: 'Данок',
            total: 'Вкупно',
            createdBy: 'Креирано од',
            orders: 'Нарачки',
            comment: 'Коментар',
            actions: 'Акции',
            warehouse: 'Магацин',
            cost: 'Трошок',
            bill: 'Плаќање',
            costType: 'Тип трошок',
            billType: 'Тип фактура',
        },
    },
};

export const i18n = createI18n({
    locale: 'en', // Set the initial locale
    fallbackLocale: 'en', // Fallback to English if translation is missing
    messages,
});

export default i18n;
