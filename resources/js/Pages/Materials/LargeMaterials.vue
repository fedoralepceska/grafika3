                    ...
                    <div class="form-container p-2 light-gray overflow-x-auto">
                        ...
                        <div>
                            <table class="excel-table mb-3">
                                ...
                                <tbody>
                                <tr v-for="m in largeMaterials.data" :key="m.id" class="clickable-row" @click="goToArticle(m)" :title="m?.article?.name ? `Open article: ${m.article.name}` : 'No linked article'">
                                    <th>{{ m?.id }}</th>
                                    <th>{{ m?.name }}</th>
                                    <th>{{ m?.quantity }}</th>
...
export default {
    ...
    methods: {
        goToArticle(material) {
            ...
            this.$inertia.visit(`/articles/${articleId}/view?from=large_materials`);
        },
        ...
        async confirmQuantityUpdate() {
            ...
            try {
                await axios.put(`/materials/large/${this.targetMaterial.id}/quantity`, {
                    quantity,
                    passcode,
                });
                await this.fetchLargeMaterials(this.largeMaterials?.current_page || 1);
                this.closeQuantityModal(true);
                toast.success('Large material quantity updated.');
            } catch (error) {
