<template>
    <MainLayout>
        <div class="pl-7 pr-7">
            <Header title="tradeInvoice" subtitle="createTradeInvoice" icon="Materials.png" link="trade-invoices"/>
            <div class="dark-gray p-5">
                <div class="form-container p-2 light-gray">
                    ...
                    <form @submit.prevent="" class="flex gap-3 justify-center overflow-x-auto">
                        <table class="excel-table">
                            ...
                            <tr>
                                ...
                                <th>{{$t('unitPrice')}} (.ден)<div class="resizer" @mousedown="initResize($event, 5)"></div></th>
                                <th>{{$t('VAT')}}%<div class="resizer" @mousedown="initResize($event, 6)"></div></th>
                                <th>{{$t('priceWithVAT')}}<div class="resizer" @mousedown="initResize($event, 7)"></div></th>
                            ...
                            <tr v-for="(row, index) in rows" :key="index">
                                ...
                                <td>
                                    <input v-model.number="row.quantity" min="0.00001" step="0.00001" type="number" class="table-input" @input="updateRowValues(index)">
                                </td>
                                <td>
                                    <input v-model.number="row.unit_price" min="0.01" step="0.01" type="number" class="table-input" @input="updateRowValues(index)">
                                </td>
                                <td>
                    ...
                    <!-- Invoice Totals -->
                    <div class="totals-section mt-4 p-4 border rounded">
                        <div class="flex justify-end">
                            <div class="totals-grid">
                                <div class="total-row">
                                    <span class="total-label">{{$t('subtotal')}}:</span>
                                    <span class="total-value">{{ formatNumber(invoiceTotals.subtotal) }} ден</span>
                                </div>
                                <div class="total-row">
                                    <span class="total-label">{{$t('VAT')}}:</span>
                                    <span class="total-value">{{ formatNumber(invoiceTotals.vat) }} ден</span>
                                </div>
                                <div class="total-row total-final">
                                    <span class="total-label">{{$t('total')}}:</span>
                                    <span class="total-value">{{ formatNumber(invoiceTotals.total) }} ден</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="notes-section mt-4">
                        <h3 class="text-white bold mb-2">{{$t('notes')}}</h3>
                        <textarea v-model="notes" class="w-full p-2 rounded text-gray-700" rows="3" :placeholder="$t('addNotesHere')"></textarea>
                    </div>

                    <div class="button-container mt-10">
                        <PrimaryButton @click="createTradeInvoice()" type="submit" :disabled="!canCreateInvoice">{{ $t('create') }}</PrimaryButton>
                    </div>
                </div>
            </div>
        </div>
    ...
    computed: {
        ...
        canCreateInvoice() {
            return this.selectedClientId && 
                   ...
                   this.rows.length > 0 && 
                   this.rows.some(row => row.article_id && row.quantity > 0);
        }
    },
    setup() {
    ...
    methods: {
        ...
        handleArticleSelected(article, index) {
            ...
            this.rows[index] = {
                ...
                code: article.code,
                name: article.name,
                available_quantity: article.available_quantity || 0,
                unit_price: article.selling_price || article.purchase_price || article.price_1,
        ...
        async createTradeInvoice() {
            ...
            const validRows = this.rows.filter(row => row.article_id && row.quantity > 0);
            if (validRows.length === 0) {
                this.toast.error('Please add at least one article with quantity');
                return;
            }
            ...
            const invoiceData = {
                ...
                invoice_date: this.selectedDate,
                notes: this.notes,
            ...
            try {
                const response = await axios.post('/trade-invoices', invoiceData);
                this.toast.success('Trade invoice created successfully!');
                this.$inertia.visit('/trade-invoices');
            } catch (error) {
                console.error('Error creating trade invoice:', error);
                this.toast.error(error.response?.data?.error || 'Failed to create trade invoice');
            }
        },
    },
};
