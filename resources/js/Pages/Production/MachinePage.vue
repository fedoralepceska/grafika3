...
export default {
    ...
    methods: {
        fetchJobs() {
            const url = `/machines/${this.machineId}/jobs`;
            axios.get(url)
                .then(response => {
        ...
        async endJob(job) {
            try {
                ...
                if (invoiceWithJob) {
                    // Check if all jobs in the invoice are completed
                    const allJobsCompleted = invoiceWithJob.jobs.every(j => j.machinePrint.every(a => a.status === 'Completed'));
                    if (allJobsCompleted) {
                        // Update the invoice status
                        await axios.put(`/orders/${invoiceWithJob.id}`, {
                            status: 'Completed',
        ...
        getOverallJobStatus() {
            const jobActionStatuses = this.jobs.flatMap((job) => job.machinePrint.map((action) => action.status.toLowerCase().trim()));
