<template>
    <div class="p-6 rounded-lg shadow-lg">
        <h2 class="text-2xl font-bold text-white mb-6">{{ $t('clientInvoiceAnalytics') }}</h2>
        ...
        <div class="flex flex-row gap-3 border-2 rounded-lg">
            ...
            <div class="w-full max-w-md mx-auto">
                <CustomDatePicker
                    @date-selected="fetchClientInvoiceCounts"
...
export default {
    ...
    setup() {
        ...
        const chartOption = computed(() => {
            const options = {
                title: {
                    text: 'Invoice Distribution by Client',
                    left: 'center',
                    top: 20,
                    textStyle: {
                        color: '#fff',
        ...
        const fetchClientInvoiceCounts = async (selectedDate) => {
            loading.value = true;
            error.value = null;
            try {
