...
async function generateA4StandardizedThumbnails(inputPath, outputDir, baseDpi) {
    try {
        ...
        // Configure pdf-poppler options with optimal DPI
        const options = {
            format: 'png',
            out_dir: outputDir,
...
async function tryA4WithPdftocairo(inputPath, outputDir, dpi) {
    try {
        ...
        console.log('Attempting A4 standardization using pdftocairo (no canvas needed)');
        ...
        // Use pdftocairo with size constraint (A4 paper option doesn't work with PNG)
        const outputPrefix = path.join(outputDir, 'thumb');
        const command = `${pdftocairoCmd} -png -r ${optimalDpi} -W 595 -H 842 "${inputPath}" "${outputPrefix}"`;
        ...
        if (files.length > 0) {
            ...
            console.log('pdftocairo executed but no files created');
            return false;
        }
